...
TEST_SUITE("VehicleIdentificationHandler") {
	 ...
	 */
	TEST_CASE_FIXTURE(VehicleIdentificationHandlerFixture, "VIN Less Than 17 Bytes") {
		// Call function under test to create message
		...
		CHECK_MESSAGE(shortVINPadded == actualVin, "Setting VIN with < 17 bytes failed");

		// Assert message after shortVin bytes is padded with '0'
		for(int i=16; i<=24; i++) {
			CHECK_EQ(message[i], '0');
			if (message[i] != '0') {
				CHECK_MESSAGE(false, "VIN not correctly padded at byte: " + std::to_string(i));
			}
		}
		delete[] message;
	}
