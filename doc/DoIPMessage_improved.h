...
public:
    ...
    explicit DoIPMessage(DoIPPayloadType pl_type, const uint8_t *data, size_t size) {
        ByteArray payload(data, data + size);
        buildMessage(pl_type, payload);
    }
...
protected:
    ...
    void buildMessage(DoIPPayloadType pl_type, const ByteArray &payload) {
        ...
        m_data.emplace_back((type_value >> 8) & 0xFF);
        m_data.emplace_back(type_value & 0xFF);
     ...
     */
    size_t getPayloadLengthFromHeader() const {
        if (m_data.size() < DOIP_HEADER_SIZE) {
            return 0;
        }
        uint32_t b3 = static_cast<uint32_t>(m_data[4]) << 24;
        uint32_t b2 = static_cast<uint32_t>(m_data[5]) << 16;
        uint32_t b1 = static_cast<uint32_t>(m_data[6]) << 8;
        uint32_t b0 = static_cast<uint32_t>(m_data[7]);
        return static_cast<size_t>(b3 | b2 | b1 | b0);
    }
};
...
namespace message {
     ...
     */
    inline DoIPMessage makeRoutingResponse(
